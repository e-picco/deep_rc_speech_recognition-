...
entity fpga2exp is
    port(
        ...
        --
        addrPattern : out unsigned(14 downto 0); --unsigned(12 downto 0);
        ...
        d_lInputs   : out unsigned(14 downto 0);
...
architecture Behavioral of fpga2exp is
    ...
    signal lInputs  : unsigned(14 downto 0); --unsigned(12 downto 0);
...
begin
    ...
    seq_inp_msk : process (clk)
        variable addrPat    : unsigned(14 downto 0); --: integer range 0 to (10*nNeurons) -1; -- -1;
        variable addrMsk    : integer range 0 to nNeurons-1;
        variable init       : std_logic;
		variable frame_cnt    : unsigned(12 downto 0); --for HAR. For SPEECH RECOGN, IT IS K (input length, NfxK)
    ...
    begin
        if rising_edge(clk) then
            if state=setup AND setTarget=linp AND inWordTrig='1' then
                lInputs <= unsigned(inWord(14 downto 0)); --unsigned(inWord(12 downto 0));
            end if;
        end if;
    end process;
