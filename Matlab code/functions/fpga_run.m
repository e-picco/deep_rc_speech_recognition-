function [ reservoir, outputs, weighted_states ] = fpga_run(p)
    ...
    switch p.fpga_data_mode
        case 'x', n_nodes = p.size_res;
        case 'xy', n_nodes = p.size_res + 1;
        case 'xw', n_nodes = 2*p.size_res;
        case 'xwy', n_nodes = 2*p.size_res + 1;
    end
...
while isempty(data)==1
    ...
    % command = 'cat \\.\xillybus_read_32 > data_rx.bin <<''EOF'' &';
    % !cat \\.\xillybus_read_32 > data_rx.bin &
...
reservoir=reshape(data_6, [p.size_res, p.n_inputs]); %NxK
% reservoir=[reservoir; p.bias_node*ones(1, p.n_inputs)];

outputs = nan(1, p.n_inputs);
weighted_states = nan(p.size_res, p.n_inputs);
